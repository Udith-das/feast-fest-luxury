...
export function InquiryForm({ compact = false }: { compact?: boolean }) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    // No backend wired — open a beautifully prefilled email so leads are never lost.
    const body = `Name: ${parsed.data.name}\nPhone: ${parsed.data.phone}\nEmail: ${parsed.data.email}\nEvent: ${parsed.data.eventType}\nDate: ${parsed.data.eventDate}\nGuests: ${parsed.data.guests}\nLocation: ${parsed.data.location}\n\nMessage:\n${parsed.data.message ?? ""}`;
    setTimeout(() => {
      window.location.href = `mailto:hello@feastandfest.com?subject=${encodeURIComponent(
        `Inquiry — ${parsed.data.eventType} for ${parsed.data.guests} guests`
      )}&body=${encodeURIComponent(body)}`;
      toast.success("Thank you. Our concierge will reach out within 24 hours.");
      setBusy(false);
      setValues({ name: "", phone: "", email: "", eventType: "", eventDate: "", guests: "", location: "", message: "" });
    }, 400);
  };
